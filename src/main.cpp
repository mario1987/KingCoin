...
#if defined(NDEBUG)
# error "Kingcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x00000010e4586fd8a68f3fe80cd260c0a2f9ffbd771679863f9bd5c4daad8a4a");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Kingcoin: starting difficulty is 1 / 2^12
...
{
    // kingcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
static const int64 nTargetTimespan = 45; // kingcoin: 45 Second Retarget
static const int64 nTargetSpacing = 45; // kingcoin: 45 Second Block Target
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    unsigned int nProofOfWorkLimit = bnProofOfWorkLimit.GetCompact();
        ...
        // kingcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // kingcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // kingcoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x00000010e4586fd8a68f3fe80cd260c0a2f9ffbd771679863f9bd5c4daad8a4a");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "October 2017 KingCoin Swap";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        assert(block.hashMerkleRoot == uint256("0xcbbbec93c9ba24409994fe91437e69dc7c2a08ecae11b5fe69c65c4727f1a1fb"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // kingcoin: increase each by adding 2 to bitcoin's value.
...
void static kingcoinMiner(CWallet *pwallet)
{
    printf("kingcoinMiner started\n");
    ...
    {
        printf("kingcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&kingcoinMiner, pwallet));
}
